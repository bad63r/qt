...
PlasmaCore.ToolTipArea {
    ...
    function numIndicatorVisibility()
    {
        if(plasmoid.configuration.separateLaunchers === false)
        {
            return true; //true
        }
        else if (pid == 0)
    ...
    Loader {
        ...
        Rectangle{
            ...
            height: 30
            color:"red"
            //visible: numIndicatorVisibility()
            visible: model.index > 8 ? false : true

            Text {
                id: name
//                text: model.index - plasmoid.configuration.launchers.length
                text: model.index +1
