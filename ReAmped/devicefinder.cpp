    ...
    m_deviceHandler(handler)
{
    ...
    m_deviceDiscoveryAgent->setLowEnergyDiscoveryTimeout(25000);
...
void DeviceFinder::startSearch()
{
    ...
    if (simulator) {
        ...
        //TOM m_deviceDiscoveryAgent->start();
...
void DeviceFinder::addDevice(const QBluetoothDeviceInfo &device)
{
    ...
    // TOM if (device.coreConfigurations()) {
...
void DeviceFinder::scanError(QBluetoothDeviceDiscoveryAgent::Error error)
{
    ...
    else if (error == QBluetoothDeviceDiscoveryAgent::NoError)
        setError(tr("0"));
